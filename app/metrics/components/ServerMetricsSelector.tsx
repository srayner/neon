...
interface ServerMetricsSelectorProps {
  servers: Server[];
  selectedServerId: number;
}

export function ServerMetricsSelector({ servers, selectedServerId }: ServerMetricsSelectorProps) {
  const router = useRouter();

  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newServerId = e.target.value;
    router.push(`/metrics?serverId=${newServerId}`);
  };
